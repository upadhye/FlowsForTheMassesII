...
double tau_t_eV(int t, double T_hdm_0_K){
...
double tau_t_eV(int t){ return tau_t_eV(t,-1); }
...
double Hc2_Hc02_eta(double eta, const struct cosmoparam C){
  ...
  //neutrinos, using relativistic Omega_hdm(eta)
...
double Tmat0(double k, const struct cosmoparam C){
  ...
  if(!init){
    ...
    while( fgets(line,sizeof line, fp) && !feof(fp)){ 
      if(*line != '#'){
        int classTncol = 0;
        double Tm = 0, classTfac = 1.0;

        switch(C.switch_transfer_type){

        case 0:  //new CAMB format: 13 columns
          sscanf(line,"%lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg",
                 t,t+1,t+2,t+3,t+4,t+5,t+6,t+7,t+8,t+9,t+10,t+11,t+12);
          Tm = t[6];
          break;

        case 1:  //old CAMB format: 7 columns
          sscanf(line,"%lg %lg %lg %lg %lg %lg %lg",t,t+1,t+2,t+3,t+4,t+5,t+6);
          Tm = t[6];
          break;

        case 2:  //CLASS format with 0 ur (ultrarelativistic) species and
                 //1 ncdm (non-cold dark matter) species
          classTncol = 9;
          sscanf(line,"%lg %lg %lg %lg %lg %lg %lg %lg %lg",
                 t,t+1,t+2,t+3,t+4,t+5,t+6,t+7,t+8);
          classTfac = -1.0 / sq(t[0]*C.h);
          for(int j=1; j<classTncol; j++) t[j] *= classTfac;
          Tm = t[5];
          break;

        case 3:  //CLASS format with 0 ur and 3 ncdm
          classTncol = 11;
        ...
        case 4:  //CLASS format with 1 ur and 1 ncdm
          classTncol = 10;
          sscanf(line,"%lg %lg %lg %lg %lg %lg %lg %lg %lg %lg",
        ...
        case 5:  //CLASS format with 1 ur and 3 ncdm
          classTncol = 12;
          sscanf(line,"%lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg",
        ...
        default:
          printf("ERROR: Invalid transfer type in parameter inputs.\n");
          fflush(stdout);
          abort();
          break;
        }
...
inline double yhdm2(int alpha, int ell, int ik, const double *y){
...
double yall(int ny, int alpha, int ell, int ik, const double *y){
  ...
  else if(alpha<N_TAU) return yhdm(ny,alpha,ell,ik,y);
...
inline double D2hdm(int alpha, int ell, int ik, const double *y){
  double k = KMIN*exp(DLNK*ik), pre = k*k*k * pre_DeltaSq;
  return pre * sq(yhdm0(alpha,ell,ik,y));
}

double D2hdmMax(int alpha, int ell, const double *y){
  double maxD2 = -1e100;
  for(int i=0; i<NK; i++) maxD2 = fmax(maxD2,D2hdm(alpha,ell,i,y));
  return maxD2;
}
...
double d_hdm_mono(const double *Ohdmt, int ik, double z, const double *y){

  static int init = 0;
  static double Omega_hdm_t_0[N_TAU];
...
double Lex(int type, int ny, int alpha, int ell, int ik, const double *y){
  ...
  if(type==1 && ell==N_MU && N_MU==2){
    return sq(yhdm(ny,alpha,ell-1,ik,y))/yhdm(ny,alpha,ell-2,ik,y);
  }
...
//compute linear evolution matrix for given hdm fluid
int compute_Xi_hdm(int alpha, double eta, int i, const double *y, 
                  const struct cosmoparam C, double Xi_hdm[2][2][2*N_MU-1]){

  double Phi = (SWITCH_HDM_SOURCE_NONLIN ? Poisson_nonlin(eta,i,y,C)
  ...
  for(int j=0; j<2*N_MU-1; j++){
    ...
    Xi_hdm[0][0][j] = kv_H * (jfacP*rP[0] - jfacN*rN[0]);
    Xi_hdm[0][1][j] = -1.0;
    Xi_hdm[1][0][j] = (j==0 ? k2_H2 * Phi / yhdm0(alpha,0,i,y) : 0);
    Xi_hdm[1][1][j] = 1.0 + dlnHc + kv_H * (jfacP*rP[1] - jfacN*rN[1]);
  }
...
double pad_power(double eta, const double *y, const struct cosmoparam C,
  ...
  int is_hdm = (alpha>=0 && alpha<N_TAU);
  if(!is_hdm && ell>0){
...
#pragma omp parallel for
  for(int i=NSHIFT+NK; i<NKP; i++){
    double lnk=LNK_PAD_MIN+DLNK*i, k=exp(lnk), kr=pow(k/KMAX,C.n_s-4.0*is_hdm);
...
double Ihdm(int alpha, int a, int c, int d, int b, int e, int f, 
  ...
  if(alpha<0 ||	alpha>=N_TAU
    ...
    printf("ERROR: Ihdm: invalid indices.  alpha=%i; acd,bef=%i%i%i,%i%i%i,",
	   alpha,a,c,d,b,e,f);
    printf(" i=%i, ell=%i\n",i,ell);
    fflush(stdout);
    abort();
  ...
  if(a==0 && c==1 && d==0) return Ihdm(alpha,a,d,c,b,f,e,i,ell,y);  
  int nI = nAI(a,c,d,b,e,f);
  if(nI<8 || nI>63) return 0;
  else if(nI<16) 
...
double Ahdm(double eta, int alpha,  int a, int c, int d,   int b, int e, int f,
           int i, int ell, const double *y, const struct cosmoparam *C){

  if(C->nAgghdm==0) return 0;
  if(ell >= 2*C->switch_Nmunl - 1) return 0;
  
  //check indices
  if(alpha<0 || alpha>=N_TAU || alpha>=C->nAgghdm
     || a<0 || a>1 || c<0 || c>1 || d<0 || d>1
     || b<0 || b>1 || e<0 || e>1 || f<0 || f>1
     || i<0 || i>=NK || ell<0 || ell>=N_MU){
    printf("ERROR: Ahdm: invalid indices.  alpha=%i; acd,bef=%i%i%i,%i%i%i,",
	   alpha,a,c,d,b,e,f);
    printf(" i=%i, ell=%i, C->nAgghdm=%i\n",i,ell,C->nAgghdm);
    fflush(stdout);
    abort();
  }
  ...
  int ifs=0;
  double sdt = pow( yhdm0(alpha,0,0,y) / yhdm0(alpha,0,0,C->yAgg), 3-b-e-f)
    * pow( yhdm1(alpha,0,0,y) / yhdm1(alpha,0,0,C->yAgg), 1+b+e+f);
  
  if(a==0 && c==0 && d==1){
    int iU = 4*b + 2*e + f;
    return sdt * C->Agghdm[alpha*N_UI*N_MU*NK + iU*N_MU*NK + ell*NK + i];
  }

  else if(a==0 && c==1 && d==0) return Ahdm(eta,alpha, a,d,c, b,f,e, i,ell,y,C);
...
int print_all(double z, const double *w){
  ...
  for(int i=0; i<NK; i++){
    ...
    for(int j=0; j<N_PI*N_TAU*N_MU + (2+N_PI+N_BISPEC); j++)
      printf(" %1.9g", w[j*NK+i]);
    printf(" %g %1.9g", ellArr[i], w[(N_PI*N_TAU*N_MU+(2+N_PI+N_BISPEC))*NK+i]);
    printf("\n");
    fflush(stdout);
  }
  printf("\n\n");
...
int print_all_y_dy_A(double z, const double *w, const double *dw,
  ...
  for(int i=0; i<NK; i++){
	...
	for(int j=0; j<N_MU; j++){
	  if(t < C->nAgghdm)
	    printf(" %e", Ahdm(eta, t, a, c, d, b, e, f, i, j, w, C));
	  else printf(" 0");
	}
      }
    }
...
//print all hdm and cb
int print_all_yhdm_ycb(double z, const double *w){
  printf("#print_all_yhdm_ycb: output at z=%e\n",z);
  for(int i=0; i<NK; i++){
    printf("%e  ", KMIN*exp(DLNK*i));
    for(int t=0; t<N_TAU; t++){
      for(int ell=0; ell<N_MU; ell++){
	printf("  %e %e %e", yhdm0(t,ell,i,w),
	       yhdm1(t,ell,i,w), yhdm2(t,ell,i,w));
      }
    }
    printf("     %e %e %e %e %e\n", ycb0l(i,w), ycb1l(i,w),
...
int print_cblin_hdmtot(int ik, double z, const double *w){
  double k = KMIN * exp(DLNK * ik);
  printf("%g %g %g %g %g\n", z, k,
...
int print_all_cblin_hdmtot(double z, const double *w){
  for(int i=0; i<NK; i++) print_cblin_hdmtot(i,z,w);
  return 0;
}

//print growth factor D, growth rate f, and total hdm growth
...
int print_all_Pcblin_Pcbnl_Phdmtot(double z, const double *w){
...
int print_all_Pcblin_Pcbnl_Phdmtot_Bispec(double z, const double *w){
...
//print dd, dt, tt monopole powers for cb(lin), cb(nl) and all hdm fluids
int print_all_Pmono(double z, const double *w){
  ...
  for(int i=0; i<NK; i++){
    ...
    for(int t=0; t<N_TAU; t++)
      printf("   %e %e %e",
             sq(yhdm0(t,0,i,w)),
             yhdm0(t,0,i,w)*yhdm1(t,0,i,w)*(1.0-yhdm2(t,0,i,w)),
             sq(yhdm1(t,0,i,w)));
    printf("\n");
    fflush(stdout);
  }
...
int print_debug(double eta, const double *w){
  ...
  for(int i=0; i<NK; i++){
    ...
    for(int alpha=0; alpha<N_TAU; alpha++){
      printf("#DEBUG PRINTOUT: call #%i: hdm(%i): ", call, alpha);
      for(int ell=0; ell<N_MU; ell++)
        printf(" %e %e %e ", yhdm0(alpha,ell,i,w),
               yhdm1(alpha,ell,i,w), yhdm2(alpha,ell,i,w));
      printf("\n");
      fflush(stdout);
    }
  }
...
int print_debug_Agghdm(double eta, const double *w,
		      const struct cosmoparam C){
  ...
  for(int t=0; t<C.nAgghdm; t++){
	...
	for(int ell=0; ell<N_MU; ell++)
	  printf(" %e",C.Agghdm[t*N_UI*N_MU*NK
			       + iU*N_MU*NK + ell*NK + i]);
	printf("  ");
      }
      printf("\n");
...
int print_debug_Xi_hdm(const double Xi_hdm[2][2][N_MU][N_MU]){

  printf("#DEBUG: Xi_hdm_ab for each ell,m\n");

  for(int ell=0; ell<N_MU; ell++){
    for(int m=0; m<N_MU; m++){
      printf("#DEBUG:Xi_hdm_ab(ell=%i,m=%i) = [ %e %e ]\n",ell,m,
	     Xi_hdm[0][0][ell][m], Xi_hdm[0][1][ell][m]);
      printf("#DEBUG:Xi_hdm                   [ %e %e ]\n",
             Xi_hdm[1][0][ell][m], Xi_hdm[1][1][ell][m]);
      fflush(stdout);
    }
  }
...
int print_menu(int ptype, double z, const double *w){
  ...
  case 1: return print_all_Pcblin_Pcbnl_Phdmtot(z,w);
